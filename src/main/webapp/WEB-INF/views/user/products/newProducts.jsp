...
<body>
	<div class="well well-small">
		...
		<div class="row-fluid">
			<ul class="thumbnails">
				<c:if test="${ allNewProductsPaginate.size() <=0 }">
					<h3 style="text-align: center; color: red;">Hiện tại chưa có
						sản phẩm này!!!</h3>
				</c:if>
				<c:if test="${ allNewProductsPaginate.size() >0 }">
					<ul class="thumbnails">
						<c:forEach var="item" items="${ allNewProductsPaginate }"
							varStatus="loop">
							<li class="span4">
								<div class="thumbnail">
									<a class="zoomTool"
							...
							<c:if
								test="${ (loop.index+1)  %3 ==0 || (loop.index+1) == allNewProductsPaginate.size()}">
					</ul>
					<c:if test="${ (loop.index+1) < allNewProductsPaginate.size()}">
						<ul class="thumbnails">
					</c:if>
				</c:if>
				</c:forEach>
	...
	<div class="pagination">
		<c:forEach var="item" begin="1"
			end="${ paginateInfoAllNewProdcts.totalPage }" varStatus="loop">
			<c:if test="${ (loop.index) == paginateInfoAllNewProdcts.currentPage }">
				<a href="<c:url value="/san-pham-moi/${ loop.index }"/>"
					class="active">${ loop.index }</a>
			</c:if>
			<c:if test="${ (loop.index) != paginateInfoAllNewProdcts.currentPage }">
				<a href="<c:url value="/san-pham-moi/${ loop.index }"/>">${ loop.index }</a>
			</c:if>
		</c:forEach>
	</div>
	<!-- </div>
